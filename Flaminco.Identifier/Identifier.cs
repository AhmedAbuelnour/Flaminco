...
public record struct Identifier<TEntity> where TEntity : notnull
{
    ...
    private Identifier(Guid value) : this()
    {
        Value = value;
    }
    ...
    {
        return $"{typeof(TEntity).Name}: {Value}";
    }
    ...
    public static implicit operator Guid(Identifier<TEntity> id)
    {
        return id.Value;
    }
    ...
    public static Identifier<TEntity> New()
    {
    ...
    /// <remarks>
    ///     This factory method simplifies the creation of an identifier for a specific entity type, allowing for direct
    ///     initialization from a <see cref="Guid" />.
    ///     It is particularly useful when working with existing GUID values that need to be converted into typed identifiers
    ///     for entities.
    /// </remarks>
    public static Identifier<TEntity> FromGuid(Guid value)
    {
        return new Identifier<TEntity>(value);
    }
}