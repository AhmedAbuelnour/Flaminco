...
public static class ManualMapperExtension
{
    ...
    public static IServiceCollection AddEntityDtoAdaptors<TScanner>(this IServiceCollection services)
    {
        IEnumerable<TypeInfo>? types = from type in typeof(TScanner).Assembly.DefinedTypes
                                       where !type.IsAbstract && type.GetInterfaces().Any(i => i.IsGenericType && (i.GetGenericTypeDefinition() == typeof(IEntityDtoAdapter<,>)))
                                       select type.GetTypeInfo();
