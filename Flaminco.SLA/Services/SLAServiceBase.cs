...
public abstract class SLAServiceBase(ILogger<SLAServiceBase> _logger)
{
    ...
    public async ValueTask<int> CalculateRemainingDays(DateOnly deliveryDate, DateOnly currentDate,
        bool addOneDayForPosVal = false)
    {
        _logger.LogInformation(
            "Calculating remaining days. DeliveryDate: {DeliveryDate}, CurrentDate: {CurrentDate}, AddOneDayForPosVal: {AddOneDayForPosVal}",
            deliveryDate, currentDate, addOneDayForPosVal);

        var totalDaysRemaining = deliveryDate.DayNumber - currentDate.DayNumber;

        var sign = totalDaysRemaining < 0 ? -1 : 1;

        var result = await GetTotalRemainingDays(Math.Abs(totalDaysRemaining), currentDate) * sign;

        if (addOneDayForPosVal && result >= 0) result++;
    ...
    {
        _logger.LogInformation(
            "Calculating delivery date. AsOfDate: {AsOfDate}, TaskDuration: {TaskDuration}, SpentDays: {SpentDays}",
            asOfDate, taskDuration, spentDays);

        //getting calendars from cache
        var calendarInfo = await GetCalendar();

        var deliveryDate = asOfDate;

        taskDuration -= spentDays;

        var sign = taskDuration < 0 ? -1 : 1;
        ...
        while (Math.Abs(taskDuration) > 0)
        {
            ...
            if (!IsWorkingDay(deliveryDate, calendarInfo) || IsHoliday(deliveryDate, calendarInfo)) continue;
    ...
    /// <returns>
    ///     A task that represents the asynchronous operation. The task result indicates whether the specified date is a
    ///     working day.
    /// </returns>
    public async ValueTask<bool> CheckIfIsWorkingDay(DateOnly date)
    {
        ...
        var result = IsWorkingDay(date, calendarInfo);
    ...
    public async ValueTask<bool> CheckIfIsHoliday(DateOnly date)
    {
        ...
        var result = IsHoliday(date, calendarInfo);
    ...
    {
        _logger.LogInformation("Calculating spent days. StartDate: {StartDate}, EndDate: {EndDate}", startDate,
            endDate);
        ...
        var totalSpentDays = Math.Max(0, endDate.Value.DayNumber - startDate.DayNumber);

        for (var i = 0; i <= totalSpentDays; i++)
        {
            if (!IsWorkingDay(startDate, calendarInfo) || IsHoliday(startDate, calendarInfo)) totalSpentDays--;
    ...
    {
        return calendarInfo.WorkingDays.Contains(date.DayOfWeek);
    }

    private static bool IsHoliday(DateOnly date, CalendarInfo calendarInfo)
    {
        return calendarInfo.Holidays.Any(vacation => date >= vacation.StartDate && date < vacation.EndDate);
    }

    private async ValueTask<int> GetTotalRemainingDays(int initialDaysRemaining, DateOnly currentDay)
    {
        _logger.LogInformation(
            "Getting total remaining days. InitialDaysRemaining: {InitialDaysRemaining}, CurrentDay: {CurrentDay}",
            initialDaysRemaining, currentDay);

        var calendarInfo = await GetCalendar();

        var totalRemainingDays = !IsWorkingDay(currentDay, calendarInfo) || IsHoliday(currentDay, calendarInfo) ? 1 : 0;

        for (var i = 0; i < initialDaysRemaining; i++)
        {
            if (IsWorkingDay(currentDay, calendarInfo) && !IsHoliday(currentDay, calendarInfo)) totalRemainingDays++;
            currentDay = currentDay.AddDays(1);
        }

        _logger.LogInformation("Total remaining days calculated. TotalRemainingDays: {TotalRemainingDays}",
            totalRemainingDays);
