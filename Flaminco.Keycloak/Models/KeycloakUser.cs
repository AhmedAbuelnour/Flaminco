...
public class KeycloakUser
{
    ...
    public Dictionary<string, IEnumerable<object>>? Attributes { get; set; }
    ...
    ///     Thrown when the format of the attribute value is invalid for the specified type
    ///     conversion.
    /// </exception>
    public T? GetAttribute<T>(string type)
    {
        ...
        if ((Attributes?.TryGetValue(type, out var value) ?? false) && value.FirstOrDefault() is object firstValue)
            try
            {
