...
internal sealed class AttachAccessTokenHandler(
    IHttpClientFactory _httpClientFactory,
    IOptions<KeycloakOptions> _keycloakOptions,
    IMemoryCache _memoryCache) : DelegatingHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var tokenResponse = await _httpClientFactory.CreateClient(Constant.KeycloakAccessTokenClient).PostAsync(
                $"realms/{_keycloakOptions.Value.Realm}/protocol/openid-connect/token", new FormUrlEncodedContent(
            ...
            if (token is null) throw new AccessTokenGenerateException();

            _memoryCache.Set(_cacheKey, token,
                new MemoryCacheEntryOptions().SetAbsoluteExpiration(TimeSpan.FromSeconds(token.ExpiresIn - 30)));
        }
