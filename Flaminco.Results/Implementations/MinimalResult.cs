...
public record MinimalResult<TValue>(
    ResultType ResultType,
    TValue? Value,
    string? ErrorCode = default,
    string? ErrorMessage = default,
...
public record MinimalResults<TValue>(
    ResultType ResultType,
...
public class Result<TReturn>
{
    ...
    public static Result<TReturn> Success(TReturn value)
    {
        return new Result<TReturn>(true, value, Error.None);
    }

    public static Result<TReturn> Failure(Error error)
    {
        return new Result<TReturn>(false, default!, error);
    }
}
