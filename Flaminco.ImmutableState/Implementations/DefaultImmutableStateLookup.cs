...
namespace Flaminco.ImmutableStates.Implementations
{
    ...
    public class DefaultImmutableStateLookup<TContext>(TContext _dbContext) : IImmutableStateLookup where TContext : DbContext
    {
        ...
        public Task<ImmutableState?> GetByCodeAsync(int code, CancellationToken cancellationToken = default)
            => _dbContext.Set<ImmutableState>().AsNoTracking().Where(a => a.Code == code).FirstOrDefaultAsync(cancellationToken);

        /// <inheritdoc/>
        public Task<ImmutableState?> GetByIdAsync(int id, CancellationToken cancellationToken = default)
            => _dbContext.Set<ImmutableState>().AsNoTracking().Where(a => a.Id == id).FirstOrDefaultAsync(cancellationToken);

        /// <inheritdoc/>
        public Task<List<ImmutableState>> GetByGroupAsync(string group, CancellationToken cancellationToken = default)
            => _dbContext.Set<ImmutableState>().AsNoTracking().Where(a => a.Group == group).ToListAsync(cancellationToken);

        /// <inheritdoc/>
        public Task<List<ImmutableState>> GetAllAsync(CancellationToken cancellationToken = default)
            => _dbContext.Set<ImmutableState>().AsNoTracking().ToListAsync(cancellationToken);

        /// <inheritdoc/>
        public Task<List<ImmutableState>> FindByPredicateAsync(Expression<Func<ImmutableState, bool>> expression, CancellationToken cancellationToken = default)
            => _dbContext.Set<ImmutableState>().AsNoTracking().Where(expression).ToListAsync(cancellationToken);
    }
}
