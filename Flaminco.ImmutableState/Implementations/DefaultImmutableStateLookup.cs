...
public class DefaultImmutableStateLookup<TContext>(TContext _dbContext)
    : IImmutableStateLookup where TContext : DbContext
{
    /// <inheritdoc />
    public Task<ImmutableState?> GetByCodeAsync(int code, CancellationToken cancellationToken = default)
    {
        return _dbContext.Set<ImmutableState>().AsNoTracking().Where(a => a.Code == code)
    ...
    {
        return _dbContext.Set<ImmutableState>().AsNoTracking().Where(a => a.Id == id)
            .FirstOrDefaultAsync(cancellationToken);
    }
    ...
    {
        return _dbContext.Set<ImmutableState>().AsNoTracking().Where(a => a.Group == group)
    ...
    {
        return _dbContext.Set<ImmutableState>().AsNoTracking().ToListAsync(cancellationToken);
    }
    ...
    {
        return _dbContext.Set<ImmutableState>().AsNoTracking().Where(expression).ToListAsync(cancellationToken);
    }
}